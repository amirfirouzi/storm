...
public class WorkerMonitor {
    ...
    public synchronized static WorkerMonitor getInstance(Map conf) {
        if (instance == null)
            instance = new WorkerMonitor(conf);
        return instance;
    }
    ...
    private WorkerMonitor(Map conf) {
        this.conf = conf;
        ...
        timeWindowSlotCount = MonitorConfiguration.getInstance(conf).getTimeWindowSlotCount();
        timeWindowSlotLength = MonitorConfiguration.getInstance(conf).getTimeWindowSlotLength();

        try {
            DataManager.getInstance(conf).checkNode(CPUInfo.getInstance().getTotalSpeed());
        } catch (Exception e) {
            logger.error(e);
...
//		return (int)((float)total / (trafficData.size() * timeWindowSlotLength));
    ...
    public void storeStats() throws Exception {
        ...
        for (TaskPair pair : trafficStats.keySet()) {
            ...
            DataManager.getInstance(conf).storeTraffic(topologyId, pair.getSourceTaskId(), pair.getDestinationTaskId(), getTraffic(pair));
        }
        ...
        for (long threadId : loadStats.keySet()) {
            ...
            DataManager.getInstance(conf).storeLoad(topologyId, executor.getBeginTask(), executor.getEndTask(), getLoad(threadId));
        }
    ...
    public void setContextInfo(TopologyContext context) {
        ...
        try {
            DataManager.getInstance(conf).checkTopology(topologyId);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
