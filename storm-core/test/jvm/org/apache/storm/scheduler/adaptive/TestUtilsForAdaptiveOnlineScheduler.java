...
public class TestUtilsForAdaptiveOnlineScheduler {
  private static int currentTime = 1450418597;

  private static final Logger LOG = LoggerFactory.getLogger(TestUtilsForAdaptiveOnlineScheduler.class);
  ...
  public static List<TopologyDetails> getListOfTopologies(Config config) {
    ...
    topos.add(TestUtilsForAdaptiveOnlineScheduler.getTopology("topo-1", config, 5, 15, 1, 1, currentTime - 2, 20));
    topos.add(TestUtilsForAdaptiveOnlineScheduler.getTopology("topo-2", config, 5, 15, 1, 1, currentTime - 8, 30));
    topos.add(TestUtilsForAdaptiveOnlineScheduler.getTopology("topo-3", config, 5, 15, 1, 1, currentTime - 16, 30));
    topos.add(TestUtilsForAdaptiveOnlineScheduler.getTopology("topo-4", config, 5, 15, 1, 1, currentTime - 16, 20));
    topos.add(TestUtilsForAdaptiveOnlineScheduler.getTopology("topo-5", config, 5, 15, 1, 1, currentTime - 24, 30));
    topos.add(TestUtilsForAdaptiveOnlineScheduler.getTopology("topo-6", config, 5, 15, 1, 1, currentTime - 2, 0));
    topos.add(TestUtilsForAdaptiveOnlineScheduler.getTopology("topo-7", config, 5, 15, 1, 1, currentTime - 8, 0));
    topos.add(TestUtilsForAdaptiveOnlineScheduler.getTopology("topo-8", config, 5, 15, 1, 1, currentTime - 16, 15));
    topos.add(TestUtilsForAdaptiveOnlineScheduler.getTopology("topo-9", config, 5, 15, 1, 1, currentTime - 16, 8));
    topos.add(TestUtilsForAdaptiveOnlineScheduler.getTopology("topo-10", config, 5, 15, 1, 1, currentTime - 24, 9));
    return topos;
  }
  ...
  public static Map<String, SupervisorDetails> genHeterogeneousSupervisors(int numSup, int portsStart) {
    return genHeterogeneousSupervisors(numSup, portsStart, 0);

  }

  public static Map<String, SupervisorDetails> genHeterogeneousSupervisors(int numSup, int portsStart, int start) {
    Map<String, SupervisorDetails> retList = new HashMap<String, SupervisorDetails>();
    Map<String, Double> resourceMap;
    double CpuStart = 150;
    double CpuIncValue = 100;
    double MemStart = 512;
    double MemIncValue = 512;

    for (int i = start; i < numSup + start; i++) {
      resourceMap = new HashMap<String, Double>();
      resourceMap.put(Config.SUPERVISOR_CPU_CAPACITY, CpuStart + (i * CpuIncValue));
      resourceMap.put(Config.SUPERVISOR_MEMORY_CAPACITY_MB, MemStart + (i * MemIncValue));
      List<Number> ports = new LinkedList<Number>();
      portsStart += 1;
      for (int j = 0; j < portsStart; j++) {
        ports.add(j);
      }
