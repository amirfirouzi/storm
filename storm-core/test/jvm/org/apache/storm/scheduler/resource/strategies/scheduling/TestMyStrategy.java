...
public class TestMyStrategy {
  ...
  public void testmyStrategy() {
    ...
    builder.setBolt("bolt-1", new TestUtilsForResourceAwareScheduler.TestBolt(),
        boltParallelism + 1).shuffleGrouping("spout");
    ...
    builder.setBolt("bolt-3", new TestUtilsForResourceAwareScheduler.TestBolt(),
        boltParallelism + 2).shuffleGrouping("bolt-2");
...
//        Map<String, SupervisorDetails> supMap = TestUtilsFormyScheduler.genHeterogeneousSupervisors(4, 4, resourceMap);
    ...
    node1.add("bolt-1");
    node1.add("bolt-2");
    Assert.assertTrue("Check DefaultResourceAwareStrategy scheduling", checkmyStrategyScheduling(nodeToComps, node1));
    ...
    Assert.assertTrue("Check DefaultResourceAwareStrategy scheduling", checkmyStrategyScheduling(nodeToComps, node2));

    //one node should have the below scheduling
    List<String> node3 = new LinkedList<>();
    node3.add("bolt-3");

    Assert.assertTrue("Check DefaultResourceAwareStrategy scheduling", checkmyStrategyScheduling(nodeToComps, node3));

    //three used and one node should be empty
    Assert.assertEquals("only three nodes should be used", 3, nodeToComps.size());
  }

  private boolean checkmyStrategyScheduling(Map<String, List<String>> nodeToComps, List<String> schedulingToFind) {
